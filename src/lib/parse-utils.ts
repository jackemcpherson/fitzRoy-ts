 ...
 * stats; FootyWire omits cells that are zero). Use this for columns the source
 * *does* track but renders blankly when zero, so the result aligns with
 * afl-api (which returns 0). Reserve {@link safeInt} (returns null) for
 * columns the source doesn't track at all.
 */
